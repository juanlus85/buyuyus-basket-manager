...
type TableSortKey = "player" | "played" | "won" | "wonPercentage" | "lost" | "lostPercentage" | "fouls" | "foulsPerGame" | "technicalFouls" | "unsportsmanlikeFouls";
...
export default function StatisticsPage() {
  ...
  const sortedPlayers = useMemo(() => [...(statistics.data?.players ?? [])].sort((left, right) => {
    const getValue = (row: NonNullable<typeof statistics.data>["players"][number]) => tableSort.key === "player"
      ...
      : tableSort.key === "foulsPerGame"
        ...
        : tableSort.key === "wonPercentage"
          ? (row.summary.played ? row.summary.won / row.summary.played : 0)
          : tableSort.key === "lostPercentage"
            ? (row.summary.played ? row.summary.lost / row.summary.played : 0)
  ...
  return <div>
    ...
    {!seasonId || seasons.isLoading ? <section className="paper-card p-8 text-sm text-muted-foreground">Cargando temporadas…</section> : statistics.isLoading ? <section className="paper-card p-8 text-sm text-muted-foreground">Calculando estadísticas de {selectedSeason?.name ?? "la temporada"}…</section> : !summary || !statistics.data?.players.length ? <section className="paper-card p-8"><p className="eyebrow">Sin datos confirmados</p><h2 className="display-face mt-1 text-3xl">Aún no hay actas para esta temporada.</h2><p className="mt-3 max-w-2xl text-sm text-muted-foreground">Cuando administración revise y confirme un acta, se incorporarán automáticamente la participación, las victorias, las derrotas y las sanciones de cada jugador.</p></section> : <>
      ...
      <section className="paper-card mt-6 overflow-hidden"><div className="border-b border-border/70 px-6 py-5"><p className="eyebrow">Detalle por jugador</p><h2 className="display-face mt-1 text-3xl">La plantilla, partido a partido</h2><p className="mt-2 text-sm text-muted-foreground">Pulsa una columna para ordenar. Un segundo toque invierte el orden. F/PJ es el promedio de faltas personales; %PG y %PP se calculan sobre los partidos jugados.</p></div><div className="overflow-x-auto"><table className="w-full min-w-[1050px] text-sm"><thead className="bg-secondary/55 text-left text-xs font-bold uppercase tracking-[.12em] text-muted-foreground"><tr><SortableHeader label="Jugador" sortKey="player" activeSort={tableSort} onSort={toggleTableSort} className="px-6 text-left" /><SortableHeader label="PJ" sortKey="played" activeSort={tableSort} onSort={toggleTableSort} /><SortableHeader label="PG" sortKey="won" activeSort={tableSort} onSort={toggleTableSort} /><SortableHeader label="%PG" sortKey="wonPercentage" activeSort={tableSort} onSort={toggleTableSort} /><SortableHeader label="PP" sortKey="lost" activeSort={tableSort} onSort={toggleTableSort} /><SortableHeader label="%PP" sortKey="lostPercentage" activeSort={tableSort} onSort={toggleTableSort} /><SortableHeader label="Faltas" sortKey="fouls" activeSort={tableSort} onSort={toggleTableSort} /><SortableHeader label="F/PJ" sortKey="foulsPerGame" activeSort={tableSort} onSort={toggleTableSort} /><SortableHeader label="Técnicas" sortKey="technicalFouls" activeSort={tableSort} onSort={toggleTableSort} /><SortableHeader label="Antidep." sortKey="unsportsmanlikeFouls" activeSort={tableSort} onSort={toggleTableSort} className="px-6" /></tr></thead><tbody>{sortedPlayers.map(row => <tr key={row.player.id} className="border-t border-border/60 transition-colors hover:bg-secondary/25"><td className="px-6 py-4"><div className="flex items-center gap-3"><span className="grid h-9 w-9 place-items-center rounded-xl bg-sidebar text-xs font-bold text-sidebar-foreground">{row.player.jerseyNumber ?? "—"}</span><div><p className="font-semibold">{row.player.shortName || row.player.fullName}</p>{row.player.shortName ? <p className="text-xs text-muted-foreground">{row.player.fullName}</p> : null}</div></div></td><ValueCell value={row.summary.played} /><ValueCell value={row.summary.won} className="text-emerald-700" /><ValueCell value={row.summary.played ? `${Math.round((row.summary.won / row.summary.played) * 100)}%` : "—"} className="text-emerald-700" /><ValueCell value={row.summary.lost} className="text-rose-700" /><ValueCell value={row.summary.played ? `${Math.round((row.summary.lost / row.summary.played) * 100)}%` : "—"} className="text-rose-700" /><ValueCell value={row.summary.fouls} /><ValueCell value={row.summary.played ? (row.summary.fouls / row.summary.played).toLocaleString("es-ES", { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : "—"} className="text-primary" /><ValueCell value={row.summary.technicalFouls} className={row.summary.technicalFouls ? "text-amber-700" : undefined} /><ValueCell value={row.summary.unsportsmanlikeFouls} className={row.summary.unsportsmanlikeFouls ? "text-rose-700" : undefined} last /></tr>)}</tbody></table></div></section>
    </>}
  </div>;
}
